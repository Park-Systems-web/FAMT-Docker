...
const LoginModal = ({
  ...
  const sendHandler = async () => {
    ...
    try {
      const res = await axios.post(
        `${process.env.API_URL}/api/mail/vcode/send`,
        {
          email: email.value,
          nation: pathname,
        },
  ...
  const nextHandler = () => {
    setLoading(true);
    axios
      .post(`${process.env.API_URL}/api/users/passwordset/check`, {
        email: email.value,
        nation: pathname,
      })
      .then((res) => {
        ...
        <DialogActions style={{ flexDirection: "column" }}>
          ...
          >
            {pathname === "eu" ? (
              ...
              <Link to="/registration" onClick={closeAllModal}>
                <Typography fontSize={smallFontSize}>
                  {createAccountText}
                </Typography>
              </Link>
            )}
            <Link to="/user/forgot-password" onClick={closeAllModal}>
              <Typography fontSize={smallFontSize}>
                {forgotPasswordText}
              </Typography>
            </Link>
